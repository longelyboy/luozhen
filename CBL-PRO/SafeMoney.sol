...
contract SafeMoney is Context, ITRC20, Ownable {
    ...
    uint256 private constant _tTotal = 10**15 * 10**8;
    ...
    uint256 constant SWAP_AND_LIQUIFY_DISABLED = 0; //交换和流动性禁用
    uint256 constant SWAP_AND_LIQUIFY_ENABLED = 1;
    uint256 constant IN_SWAP_AND_LIQUIFY = 2;
    uint256 LiqStatus = SWAP_AND_LIQUIFY_ENABLED;  //流动性状态

    uint256 public _maxTxAmount = ~uint256(0);
    uint256 public numTokensSellToAddToLiquidity = 5* 10**10 * 10**8;

    string private constant _name = 'Safe Money'; 
    string private constant _symbol = 'SafeMoney';
