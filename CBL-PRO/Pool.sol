...
// 合约地址 0x0cEC1A9154Ff802e7934Fc916Ed7Ca50bDE6844e  etherscan
...
contract Pool {
    ...
    /// @notice Cap on the percentage of totalSupply that can be minted at each mint 每个铸币厂可以制造的总供应量的百分比上限
    ...
    /// @notice A record of each accounts delegate 每个帐户委托的记录
    mapping (address => address) public delegates;

    /// @notice A checkpoint for marking number of votes from a given block 标记给定块中的投票数的检查点
    struct Checkpoint {
        uint32 fromBlock;
        uint96 votes;  //投票
    }

    /// @notice A record of votes checkpoints for each account, by index 每个帐户的投票检查点的记录，按索引
    mapping (address => mapping (uint32 => Checkpoint)) public checkpoints;

    /// @notice The number of checkpoints for each account  每个帐户的检查点数目
    mapping (address => uint32) public numCheckpoints;

    /// @notice The EIP-712 typehash for the contract's domain
    bytes32 public constant DOMAIN_TYPEHASH = keccak256("EIP712Domain(string name,uint256 chainId,address verifyingContract)");

    /// @notice The EIP-712 typehash for the delegation struct used by the contract 契约使用的委托结构的EIP-712类型散列
    bytes32 public constant DELEGATION_TYPEHASH = keccak256("Delegation(address delegatee,uint256 nonce,uint256 expiry)");

    /// @notice The EIP-712 typehash for the permit struct used by the contract 授权合约
    bytes32 public constant PERMIT_TYPEHASH = keccak256("Permit(address owner,address spender,uint256 value,uint256 nonce,uint256 deadline)");

    /// @notice A record of states for signing / validating signatures  用于签名/验证签名的状态记录
    ...
    /// @notice An event thats emitted when a delegate account's vote balance changes 当委托帐户的投票平衡发生变化时触发的事件
    ...
    constructor(address account, address minter_, uint mintingAllowedAfter_) public {
        require(mintingAllowedAfter_ >= block.timestamp, "Pool::constructor: minting can only begin after deployment");  //挖矿只能在部署之后开始
    ...
    function setMinter(address minter_) external {
        require(msg.sender == minter, "Pool::setMinter: only the minter can change the minter address"); //拥有者才能更新地址
    ...
    function mint(address dst, uint rawAmount) external {
        ...
        // record the mint  设置下次增发时间 1year
        mintingAllowedAfter = SafeMath.add(block.timestamp, minimumTimeBetweenMints);

        // mint the amount
        uint96 amount = safe96(rawAmount, "Pool::mint: amount exceeds 96 bits");
        require(amount <= SafeMath.div(SafeMath.mul(totalSupply, mintCap), 100), "Pool::mint: exceeded mint cap"); //增发数量不能超过 2%
     ...
     * @return The number of tokens approved 授权金额
     */
    ...
    /**
     * @notice Approve `spender` to transfer up to `amount` from `src`  授权
    ...
    /**
     * @notice Triggers an approval from owner to spends 触发业主对支出的批准
    ...
    function permit(address owner, address spender, uint rawAmount, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        ...
        address signatory = ecrecover(digest, v, r, s);  //基于椭圆曲线签名找回与指定公钥关联的地址，发生错误的时候返回 0
     ...
     * @param delegatee The address to delegate votes to  将选票委托给的地址
     */
    ...
    /**
     * @notice Delegates votes from signatory to `delegatee` 从签署人到“被授权人”的代表投票  线下签名 减少GAS
     * @param delegatee The address to delegate votes to 将选票委托给的地址
     * @param nonce The contract state required to match the signature  要求与签名匹配的合同状态
     * @param expiry The time at which to expire the signature 签名的过期时间
     * @param v The recovery byte of the signature
     * @param r Half of the ECDSA signature pair
     * @param s Half of the ECDSA signature pair
     */
    ...
    /**
     * @notice Gets the current votes balance for `account`  获取“帐户”的当前投票余额。
    ...
    /**
     * @notice Determine the prior number of votes for an account as of a block number 根据区块号确定帐户的先前投票数
     * @dev Block number must be a finalized block or else this function will revert to prevent misinformation. 块号必须是最终确定的块，否则此函数将恢复以防止错误信息。
     * @param account The address of the account to check
     * @param blockNumber The block number to get the vote balance at
     * @return The number of votes the account had as of the given block 在给定的区块中，帐户拥有的投票数
     */
    function getPriorVotes(address account, uint blockNumber) public view returns (uint96) {
        ...
        // First check most recent balance 首先检查最近的余额
        if (checkpoints[account][nCheckpoints - 1].fromBlock <= blockNumber) {
            return checkpoints[account][nCheckpoints - 1].votes;
        }

        // Next check implicit zero balance  接下来检查隐式零余额
    ...
    // _delegate  委托  delegator  msg.sender
    ...
    // srcRep  转出地址的委托人  dstRep  转入地址的委托人  

    function _moveDelegates(address srcRep, address dstRep, uint96 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                uint32 srcRepNum = numCheckpoints[srcRep];
                uint96 srcRepOld = srcRepNum > 0 ? checkpoints[srcRep][srcRepNum - 1].votes : 0;  //获取之前的投票数
                uint96 srcRepNew = sub96(srcRepOld, amount, "Pool::_moveVotes: vote amount underflows");  //扣除数量后新的投票数
